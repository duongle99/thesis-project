...
def index():
    if request.method == 'POST':
        ...
        # Create a folder on Google Drive with the name as per the registration name
        drive_service = build('drive', 'v3', credentials=credentials)
        folder_id = '1953qkScImT2ECzVBE2MPTicRyCFTCfe7'

        detector = MTCNN()

        for i, photo_data_url in enumerate(photo_data_urls):
            photo_data = base64.b64decode(photo_data_url.split(",")[1])
            photo_filename = f"{mssv.replace(' ', '_')}_{i+1}.png"  # Change the file extension to PNG
            photo_path = os.path.join("photos", photo_filename)
            with open(photo_path, "wb") as photo_file:  # Open the file in binary mode
                photo_file.write(photo_data)

            image = Image.open(photo_path).convert("RGB")  # Convert to RGB color mode
            image_array = np.array(image)
            faces = detector.detect_faces(image_array)

            if len(faces) > 0:
                x, y, w, h = faces[0]['box']
                face_image = image.crop((x-w*1.5, y-h*0.5, x+w*1.5, y+h*1.5))

                # Check if the image contains a detected face
                face_detected = True

                # Perform additional checks on the face image if needed
                # ...

                if face_detected:
                    face_image.save(photo_path, "PNG")  # Save the image in PNG format
